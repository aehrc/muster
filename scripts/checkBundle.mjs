...
// and `net` without the `node:` prefix, which is legitimate; a check that only
// accepted the prefixed form would reject them.
//
...
// That is not hypothetical: dependency trees contain messages such as "Cannot get
// 'saltLength' from 'alg' argument", and reading that as an import of a module
// called `alg` fails the build for a dependency that bundles perfectly well. A
// false positive is the more dangerous of the two failures, because the obvious
...
// its own and is matched directly. Candidates are then filtered to things that
// could be a module specifier at all. `bundleCheck.test.ts` in `apps/server`
...
const PATTERNS = [
  ...
  // and a sentence quoting the line to add would otherwise read as one.
  /(?:^|[;}\n])\s*import\s*["']([^"'\n]+)["']/gm,
  /\brequire\(\s*["']([^"'\n]+)["']\s*\)/g,
  /\bimport\(\s*["']([^"'\n]+)["']\s*\)/g,
];
