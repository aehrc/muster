...
// them gives the figure the quality gate in CLAUDE.md names.
//
...
// total. The gate is lines and functions.
//
...
// Arguments are consumed in order rather than filtered, so that `--floor` and the
// report path may appear on either side of each other. Filtering the flags out and
// then indexing what is left cannot tell the flag's value from a positional
// argument: `checkCoverage.mjs lcov.info --floor 0.4` leaves `["lcov.info", "0.4"]`,
// and taking the second of those reads the floor as the report path - which fails
// with "0.4 was not written" rather than checking anything.
...
const report = positional[0] ?? "coverage/lcov.info";

if (unrecognised || !Number.isFinite(floor) || floor <= 0 || floor > 1) {
  console.error(
    "usage: node scripts/checkCoverage.mjs [lcov.info] [--floor 0.8]",
  );
  process.exit(2);
