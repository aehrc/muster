 ...
 *
 * The driver hands results back as `any`, which would spread through every
 * function in this module. This is the one place that boundary is crossed: the
 * result is narrowed to an array of raw rows here, and the mappers below turn
 * those into Muster's own types.
 *
 ...
 * ```
 */
const queryRows = async (result: unknown): Promise<RawRow[]> => {
 ...
 */
const arrayLiteral = (values: readonly string[]): string =>
  `{${values
    .map(
      (value) =>
